...
// Тип для конфигурации уровня уравнений
export interface EquationsLevelConfig {
  level: number;
  minScore: number;
  maxScore: number;
  maxNumber: number;
  equationTypes: Array<'addition' | 'subtraction' | 'reverseSubtraction'>;
  description: string;
  pointsPerCorrect: number;
}

// Тип для информации о следующем уровне
...
export interface EquationProblem {
  ...
  options: (string | number)[];
  correctIndex: number;
  levelConfig: EquationsLevelConfig;
  xValue: number;
}
