  ...
  <div class="grade-selection-overlay">
    <div class="grade-selection-container">
      <div class="grade-selection-header">
        ...
        <p class="grade-selection-subtitle">
          Пожалуйста, выберите ваш класс, чтобы мы могли настроить задания под ваш уровень
        </p>
      </div>
...
<script setup lang="ts">
  ...
  const emit = defineEmits<{
    gradeSelected: [grade: GradeLevel];
  }>();
  const settingsStore = useSettingsStore();
  const selectedGrade = ref<GradeLevel | null>(null);

  // Доступные классы
  const availableGrades: GradeOption[] = [
  ...
  const confirmGradeSelection = (grade: GradeLevel): void => {
